...
# ========== ПАТЧ ДЛЯ ОБХОДА ОШИБКИ "header too long" ==========
# Сохраняем оригинальный класс парсера
_original_bytes_parser = email.parser.BytesParser

# Создаём подкласс с увеличенным лимитом длины строки заголовка
class _PatchedBytesParser(_original_bytes_parser):
    def __init__(self, *args, **kwargs):
        # Создаём политику с отключённым ограничением
        custom_policy = email.policy.default.clone(max_line_length=None)
        # Передаём её в родительский конструктор
        super().__init__(*args, policy=custom_policy, **kwargs)

# Подменяем парсер в модуле email.parser
email.parser.BytesParser = _PatchedBytesParser
# ===============================================================

# Теперь импортируем imap_tools (после подмены)
from imap_tools import MailBox, AND, A

# Подключаем модули классификации
try:
